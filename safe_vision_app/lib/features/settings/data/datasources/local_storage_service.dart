...
class LocalStorageService {
  ...
  static const _keyBasicDisplayMode = 'basic_display_mode';
  ...
  Future<bool> getBasicDisplayModeEnabled() async {
    final p = await _prefs;
    return p.getBool(_keyBasicDisplayMode) ?? AppConstants.basicModeDefaultEnabled;
  }

  Future<void> setBasicDisplayModeEnabled(bool v) async {
    final p = await _prefs;
    await p.setBool(_keyBasicDisplayMode, v);
  }
  ...
  Future<void> setTtsLanguage(String lang) async {
    ...
    await p.setString(
      _keyTtsLanguage,
      lang.isEmpty ? AppConstants.ttsLanguage : lang,
    );
  }
}
