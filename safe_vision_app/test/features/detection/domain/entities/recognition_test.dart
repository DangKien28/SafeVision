...
void main() {
  ...
  group('Recognition', () {
    test('stores all properties', () {
      const r = Recognition(id: 1, label: 'xe', score: 0.9, location: testBox);
      expect(r.id, 1);
      expect(r.label, 'xe');
      expect(r.score, 0.9);
    ...
    test('toDetectionObject converts correctly', () {
      const r =
          Recognition(id: 1, label: 'balo', score: 0.85, location: testBox);
      final obj = r.toDetectionObject();
      expect(obj.label, 'balo');
      expect(obj.confidence, 0.85);
      expect(obj.boundingBox, testBox);
    });

    test('Equatable equality', () {
      const r1 = Recognition(id: 1, label: 'xe', score: 0.9, location: testBox);
      const r2 = Recognition(id: 1, label: 'xe', score: 0.9, location: testBox);
      const r3 = Recognition(id: 2, label: 'xe', score: 0.9, location: testBox);
      expect(r1, equals(r2));
    ...
    test('props contains all fields', () {
      const r = Recognition(id: 1, label: 'xe', score: 0.9, location: testBox);
      expect(r.props, [1, 'xe', 0.9, testBox]);
    });
  });
}
